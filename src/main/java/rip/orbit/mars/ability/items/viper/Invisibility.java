...
public class Invisibility extends Ability {
	...
	public String displayName() {
		return CC.translate("&b&lInvisibility &7(Viper)");
	}
	...
	public void onBr0Invis(PlayerInteractEvent event) {
		...
		if (isSimilar(event.getItem())) {
			...
			List<String> hitMsg = Arrays.asList(
					" ",
					"&bYou" + " &fhave just used a " + displayName(),
					Symbols.STAR + "&fYou have &82 minutes&f of &8full invisibility&f.",
					Symbols.STAR + "&fIf you're hit whilst this is activated, you will",
					Symbols.STAR + "&fbe shown and no longer invisible to people.",
					" ");
	...
	public void hidePlayer(Player player, int duration) {
		...
		}.runTaskLater(Mars.getInstance(), 20 * duration);
	}
