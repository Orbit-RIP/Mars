...
public class Curse extends Ability {
	...
	public String displayName() {
		return CC.translate("&2&lInventory Curse");
	}
	...
	public void onHit(EntityDamageByEntityEvent event) {
		...
		if (!hits.isEmpty() && hits.get(damager.getUniqueId()) != null && hits.get(damager.getUniqueId()) >= 3) {
			...
			List<String> beenHitMsg = Arrays.asList(
					...
					" ",
					"&6&l┃ &fYour pots are now clickable soups",
					"&6&l┃ &ffor &215 seconds&f.",
					"");
			...
			List<String> hitMsg = Arrays.asList(
					...
					" ",
					"&6&l┃ &fTheir pots are now clickable soups",
					"&6&l┃ &ffor &215 seconds&f.",
					"");
		...
		if (souped.onCooldown(event.getPlayer())) {
			...
			if (player.getHealth() <= 19 && !soupCooldown.onCooldown(player)) {
				...
				if (soupCooldown.onCooldown(player)) {
					player.sendMessage(CC.translate("&cYou cannot soup for " + soupCooldown.getRemaining(player)));
				} else {
					player.sendMessage(CC.translate("&cYou have to lose health before you do that!"));
				}
			}
		}
	}
