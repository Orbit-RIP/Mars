...
import rip.orbit.mars.ability.event.ReconPlayerFoundEvent;
...
public class Recon extends Ability {
	...
	public String displayName() {
		return CC.translate("&d&lRecon Ability");
	}
	...
	public void onInteract(PlayerInteractEvent event) {
		...
		if (isSimilar(event)) {
			...
			if (foundNames.isEmpty()) {
				p.sendMessage(CC.translate("&d&lPlayer's Found"));
				foundNames.forEach(s -> {
					p.sendMessage(CC.translate("&7» &dNone"));
				});
			} else {
				p.sendMessage(CC.translate("&d&lPlayer's Found"));
				foundNames.forEach(s -> {
					...
					p.sendMessage(CC.translate("&7» &d" + s.getName() + " &7(" + x + ", " + y + ", " + z + ")"));
				});
			}
			p.sendMessage(" ");
			p.sendMessage(CC.translate("&7&oIf you're on LC/CB the Waypoints will be deleted in 15 seconds."));
		}
	}

	@EventHandler
	public void onPlayerFoundEvent(ReconPlayerFoundEvent event) {
		event.getPlayer().sendMessage(CC.translate("&c" + event.getUser().getName() + " has just used a " + displayName() + " &cand your location is now visible to them."));
	}
