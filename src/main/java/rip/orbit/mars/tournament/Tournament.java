...
public class Tournament {
    ...
    public void check() {
        ...
        if (activeParties.isEmpty()) {
            if (lost.isEmpty()) {
                ...
                Mars.getInstance().getTournamentHandler().setTournament(null);
                return;
            }

            // shouldn't happen, meant that the two last parties disconnected at the last second
            Bukkit.broadcastMessage(CC.translate("&cThe tournament's last two teams forfeited. Winner by default: " + PatchedPlayerUtils.getFormattedName((lost.get(lost.size() - 1)).getLeader()) + "'s team!"));
            Mars.getInstance().getTournamentHandler().setTournament(null); // Removes references to this tournament, will get cleaned up by GC
            stage = TournamentStage.FINISHED;
            return;
        }
        ...
        if (activeParties.size() == 1) {
            ...
            if (party.getMembers().size() == 1) {
                repeatMessage(CC.translate("&6&l" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + " &7won the tournament!"), 4, 2);
                ...
                repeatMessage(CC.translate("&6&l" + PatchedPlayerUtils.getFormattedName(members[0]) + " &7and &6&l" + PatchedPlayerUtils.getFormattedName(members[1]) + " &7won the tournament!"), 4, 2);
            } else {
                repeatMessage(CC.translate("&6&l" + PatchedPlayerUtils.getFormattedName(party.getLeader()) + "&7's team won the tournament!"), 4, 2);
            }
        ...
        if (--beginNextRoundIn >= 1) {
            ...
            case 1:
                if (currentRound == 0) {
                    Bukkit.broadcastMessage(CC.translate("&7The &6&ltournament &7will begin in &6" + beginNextRoundIn + " &7second" + (beginNextRoundIn == 1 ? "" : "s") + "."));
                } else {
                    Bukkit.broadcastMessage(CC.translate("&6&lRound " + (currentRound + 1) + " &7will begin in &6" + beginNextRoundIn + " &7second" + (beginNextRoundIn == 1 ? "" : "s") + "."));
                }
            }
    ...
    private void checkActiveParties() {
        Set<UUID> realParties = Mars.getInstance().getPartyHandler().getParties().stream().map(p -> p.getPartyId()).collect(Collectors.toSet());
    ...
    private void repeatMessage(String message, int times, int interval) {
        ...
        }.runTaskTimer(Mars.getInstance(), 0, interval * 20);
    }
    ...
    private void startRound() {
        ...
        Bukkit.broadcastMessage(CC.translate("&6&lRound " + ++currentRound + " &7has begun. Good luck!"));
        Bukkit.broadcastMessage(CC.translate("&7You can use &6/status &7to see who is fighting."));
        ...
        while (1 < oldPartyList.size()) {
            ...
            matches.add(Mars.getInstance().getMatchHandler().startMatch(ImmutableList.of(new MatchTeam(firstParty.getMembers()), new MatchTeam(secondParty.getMembers())), type, false, false));
        }
    ...
    public void broadcastJoinMessage() {
        ...
        Bukkit.broadcastMessage(CC.translate("&7A &6&ltournament&7 has started. Type &6/join&7 to play. (" + (this.activeParties.size() * multiplier) + "/" + (requiredTeams * multiplier) + ")"));
        Bukkit.broadcastMessage("");
    }
    ...
    private void broadcastJoinMessage(Party joiningParty) {
        if (getCurrentRound() != -1) {
            ...
            if (joiningParty.getMembers().size() == 1) {
                message = new FancyMessage(CC.translate("&6&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7 &7has &7joined &7the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            } else if (joiningParty.getMembers().size() == 2) {
                Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
                message = new FancyMessage(CC.translate("&6&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 &7and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have joined the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
            } else {
                message = new FancyMessage(CC.translate("&6&lDONOR ONLY &7- " + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7's team has joined the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
            }

            message.tooltip(CC.translate("&6Donators &7can join during the tournament countdown. Purchase a rank at &6 " + (Mars.getInstance().getDominantColor() == ChatColor.LIGHT_PURPLE ? "donate.orbit.rip" : "donate.orbit.rip") +  " &7."));
        ...
        FancyMessage message;
        if (joiningParty.getMembers().size() == 1) {
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7 has joined the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        } else if (joiningParty.getMembers().size() == 2) {
            Iterator<UUID> membersIterator = joiningParty.getMembers().iterator();
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 have joined the &6tournament&7. &7(" + activeParties.size() * 2 + "/" + requiredPartiesToStart * 2 + "&7)"));
        } else {
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(joiningParty.getLeader()) + "&7's team has joined the &6tournament&7. &7(" + activeParties.size() + "/" + requiredPartiesToStart + "&7)"));
        }
        
        message.command("/djm");
        message.tooltip(CC.translate("&6&lCLICK &7to hide this message."));
    ...
    private void broadcastEliminationMessage(Party loserParty) {
        ...
        if (loserParty.getMembers().size() == 1) {
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(loserParty.getLeader()) + "&7 has been eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        } else if (loserParty.getMembers().size() == 2) {
            Iterator<UUID> membersIterator = loserParty.getMembers().iterator();
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + "&7 and &6" + PatchedPlayerUtils.getFormattedName(membersIterator.next()) + " &7were eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        } else {
            message = new FancyMessage(CC.translate("&6" + PatchedPlayerUtils.getFormattedName(loserParty.getLeader()) + "&7's team has been eliminated. &7(" + activeParties.size() * multiplier + "/" + requiredPartiesToStart * multiplier + "&7)"));
        }
    ...
    public static void joinMessages(Player sender) {
        boolean oldValue = Mars.getInstance().getSettingHandler().getSetting(sender, Setting.SEE_TOURNAMENT_JOIN_MESSAGE);
        if (!oldValue) {
            sender.sendMessage(ChatColor.RED + "You have already disabled tournament join messages.");
            return;
        }

        Mars.getInstance().getSettingHandler().updateSetting(sender, Setting.SEE_TOURNAMENT_JOIN_MESSAGE, false);
    ...
    public static void eliminationMessages(Player sender) {
        boolean oldValue = Mars.getInstance().getSettingHandler().getSetting(sender, Setting.SEE_TOURNAMENT_ELIMINATION_MESSAGES);
        if (!oldValue) {
            sender.sendMessage(ChatColor.RED + "You have already disabled tournament elimination messages.");
            return;
        }

        Mars.getInstance().getSettingHandler().updateSetting(sender, Setting.SEE_TOURNAMENT_ELIMINATION_MESSAGES, false);
