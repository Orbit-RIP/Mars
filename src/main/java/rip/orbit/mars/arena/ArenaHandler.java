...
import rip.orbit.mars.arena.event.ArenaAllocatedEvent;
import rip.orbit.mars.arena.event.ArenaReleasedEvent;
import rip.orbit.mars.arena.listener.ArenaItemResetListener;
...
public final class ArenaHandler {
    ...
    public ArenaHandler() {
        Bukkit.getPluginManager().registerEvents(new ArenaItemResetListener(), Mars.getInstance());
        ...
        try {
            ...
            if (arenaInstancesFile.exists()) {
                try (Reader arenaInstancesReader = Files.newReader(arenaInstancesFile, Charsets.UTF_8)) {
                    ...
                    List<Arena> arenaList = Mars.getGson().fromJson(arenaInstancesReader, arenaListType);
            ...
            if (schematicsFile.exists()) {
                try (Reader schematicsFileReader = Files.newReader(schematicsFile, Charsets.UTF_8)) {
                    ...
                    List<ArenaSchematic> schematicList = Mars.getGson().fromJson(schematicsFileReader, schematicListType);
    ...
    public void saveSchematics() throws IOException {
        Files.write(
            Mars.getGson().toJson(schematics.values()),
     ...
     * @Parameter schematic ArenaSchematic to use when looking up arena
     * @Parameter copy copy of arena to look up
     ...
     * @Parameter schematic schematic to look up arenas for
     ...
     * @Parameter schematic schematic to count arenas for
     ...
     * @Parameter schematicName schematic id to search with
     ...
     * automatically.
     * @Parameter acceptableSchematicPredicate Predicate to use to determine if an {@link ArenaSchematic}
     *                                     is eligible for use.
    ...
    public Optional<Arena> allocateUnusedArena(Predicate<ArenaSchematic> acceptableSchematicPredicate) {
        ...
        Arena selected = acceptableArenas.get(Proton.RANDOM.nextInt(acceptableArenas.size()));
     ...
     * @Parameter arena the arena to release
     */
