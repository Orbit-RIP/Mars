package rip.orbit.mars.pvpclasses;

import lombok.Getter;
import rip.orbit.mars.Mars;
import rip.orbit.mars.kittype.KitType;
import rip.orbit.mars.match.Match;
import rip.orbit.mars.pvpclasses.event.BardRestoreEvent;
...
public class PvPClassHandler extends BukkitRunnable implements Listener {
    ...
    public PvPClassHandler() {
        ...
        for (PvPClass pvpClass : pvpClasses) {
            Mars.getInstance().getServer().getPluginManager().registerEvents(pvpClass, Mars.getInstance());
        }

        Mars.getInstance().getServer().getScheduler().runTaskTimer(Mars.getInstance(), this, 2L, 2L);
        Mars.getInstance().getServer().getPluginManager().registerEvents(this, Mars.getInstance());
    }

    @Override
    public void run() {
        for (Player player : Mars.getInstance().getServer().getOnlinePlayers()) {
